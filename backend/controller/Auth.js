...
const login = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign(
            { userId: user._id, mobileno: user.mobileno, userType: user.userType },
        ...
        res.status(200).json({
            ...
            user: {
                ...
                email: user.email,
                userType: user.userType, // Include user type
            }
        });
    } catch (error) {
...
const register = async (req, res) => {
    try {
        ...
        console.log(name, email, password, mobileno, userType);
        if (!name || !email || !password || !mobileno || !userType) {
            return res.status(400).json({ message: "Enter all the fields" });
        }

        // Validate userType is either 'farmer' or 'buyer'
